...
- 观察不是完整地图，而是以智能体为中心的局部小网格，默认 `7x7`，界面可切到 `9x9`。
- observation 里包含局部地形、当前朝向、上一步动作、上一步奖励、是否有钥匙，以及最近访问/重复访问提示。
- 默认使用 `RecurrentPPO + CnnLstmPolicy`，并配套一个适合小网格的轻量 CNN 特征提取器；仍可通过 `--algo ppo` 退回普通 PPO。
...
- `算法`：切换 `Recurrent` 或 `普通PPO`。默认 `Recurrent`，也就是带 LSTM 隐状态的 Recurrent PPO。
- `课程`：切换课程学习开启或关闭。默认开启，先学简单出口，再学钥匙门，最后混合复杂地图。
- `观察`：切换 `网格CNN` 或旧版 `四向带`。默认 `网格CNN`。
- `视野`：切换局部窗口大小，默认 `7x7`，也可切换到 `9x9`。
- `尺寸`：切换随机地图尺寸，例如 `9x11 / 11x15 / 13x17 / 15x21 / 17x25 / 21x31`。
- `风格`：切换随机地图布局，包含混合、开放、分割、房间、死胡同、迷宫。
- `门方向`：切换门所在墙的方向，包含混合、横墙、竖墙。
...
- `观察 / 视野`：切换试验场使用的 observation 格式和局部窗口大小，需要和训练模型保持一致。
- `尺寸 / 风格 / 门方向 / 位置 / 墙 / 陷阱`：调整之后新生成的随机图参数；已经生成的随机图历史不会被改写。
...
试验场回放时，暗色区域代表智能体当前看不到的位置。PPO 模型实际收到的 observation 也只包含局部 `7x7` 或 `9x9` 小网格，不包含完整地图。

视野规则：智能体不会看到完整地图；默认只能看到以自己为中心的 `7x7` 局部窗口。模型输入是 7 通道小图，包含地形编码、最近访问格子、当前朝向、上一步动作、上一步奖励、钥匙状态和重复访问提示。旧版四向视野带仍可用 `--obs-mode strips` 保留。

注意：当前默认模型已经是 Recurrent PPO，模型本身会维护 LSTM 隐状态。界面里的“记忆辅助”仍然只是试验场回放层的短期辅助逻辑，用来对比或兜底；如果要观察模型本身能力，可以在试验场关闭“记忆辅助”。

如果之前训练过旧版“四向单线视野”或“四向窄带”的模型，需要在训练场点击 `重置智能体` 后重新训练。新版默认 observation 已经变成 `7x7` 小网格，旧模型不能直接用于新版试验场。
...
```bash
cd /Users/ericpan/game_project/deep_learning/ai-maze-ppo-lab
python train_ppo.py --timesteps 1000000 --n-envs 4 --random-maps 200 --ent-coef 0.03 --obs-mode grid --local-view-size 7 --random-rows 11 --random-cols 15 --wall-density 0.12 --trap-density 0.04
```
...
回放窗口会显示完整地图，并用暗色遮罩标出当前智能体看不到的区域。PPO 模型实际收到的 observation 只包含局部 `7x7` 或 `9x9` 小网格，不包含完整地图。
...
- Recurrent CNN PPO 比普通 MLP PPO 更适合局部视野，但依然需要足够训练步数；复杂随机泛化通常要百万级到数百万步。
...
- 钥匙奖励不会重复刷。
- 到达出口会成功结束。
- 局部小网格 observation 形状固定，并包含上一步动作和重复访问提示。
- 随机地图包含 `S / K / D / E` 且可解。
