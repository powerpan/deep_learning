...
- `并行`：切换 PPO 采样环境数量，包含 `1 / 2 / 4 / 6`。MacBook Air M4 16GB 推荐先用 `4`，需要留出系统余量时不要开到更高。
...
训练脚本会默认限制 PyTorch/OpenMP 线程数，避免每个环境再额外开很多线程造成过载。右下角界面日志只显示训练进度、episode 数和最近 100 局成功率，不显示 PPO 内部 loss 表格。
